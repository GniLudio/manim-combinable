...
class FadeOut(AnimationGroup):
    """Replaces :class:`manim.FadeOut`"""

    def __init__(
        self,
        *mobjects: Mobject,
        opacity: float | tuple[float | None, float | None] | None = 0,
        ...
        **kwargs: Any,
        ...
        if shift is not None:
            animations.append(Shift(mobject=mobject, shift=shift))
        if target_position is not None:
            animations.append(MoveTo(mobject=mobject, point_or_mobject=target_position))
        if scale is not None:
            animations.append(Scale(mobject=mobject, scale_factor=scale))
